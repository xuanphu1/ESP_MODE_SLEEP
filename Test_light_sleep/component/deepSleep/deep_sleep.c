...
/// @brief print the source wakeup and go into deep sleep state
/// @param  
void deep_sleep(void ) //*args
{
    /**
...
//------------------------------Timer wake up------------------------------------------------------------------------------
void deep_sleep_register_rtc_timer_wakeup(int wakeup_time_usec)
{
    printf("Enabling timer wakeup, %dus\n", wakeup_time_usec);
    ESP_ERROR_CHECK_WITHOUT_ABORT(esp_sleep_enable_timer_wakeup(wakeup_time_usec));
}
...
void deep_sleep_register_gpio_wakeup(void)
{
    ...
    ESP_ERROR_CHECK_WITHOUT_ABORT(esp_deep_sleep_enable_gpio_wakeup(BIT(DEFAULT_WAKEUP_PIN), DEFAULT_WAKEUP_LEVEL));
...
void deep_sleep_register_ext0_wakeup(void)
{
    ...
    ESP_ERROR_CHECK_WITHOUT_ABORT(esp_sleep_enable_ext0_wakeup(ext_wakeup_pin_0, 1));

    // Configure pullup/downs via RTCIO to tie wakeup pins to inactive level during deepsleep.
    // EXT0 resides in the same power domain (RTC_PERIPH) as the RTC IO pullup/downs.
    // No need to keep that power domain explicitly, unlike EXT1.
    ESP_ERROR_CHECK_WITHOUT_ABORT(rtc_gpio_pullup_dis(ext_wakeup_pin_0));
    ESP_ERROR_CHECK_WITHOUT_ABORT(rtc_gpio_pulldown_en(ext_wakeup_pin_0));
}
#endif // CONFIG_EXAMPLE_EXT0_WAKEUP
...
void deep_sleep_register_ext1_wakeup(void)
{
    ...
    ESP_ERROR_CHECK_WITHOUT_ABORT(esp_sleep_enable_ext1_wakeup_with_level_mask(ext_wakeup_pin_1_mask | ext_wakeup_pin_2_mask, ext_wakeup_mode));
#else
    const esp_sleep_ext1_wakeup_mode_t ext_wakeup_mode = CONFIG_EXAMPLE_EXT1_WAKEUP_MODE;
    ESP_ERROR_CHECK_WITHOUT_ABORT(esp_sleep_enable_ext1_wakeup(ext_wakeup_pin_1_mask | ext_wakeup_pin_2_mask, ext_wakeup_mode));
#endif
    ...
    if (CONFIG_EXAMPLE_EXT1_WAKEUP_MODE_PIN_1) {
        ESP_ERROR_CHECK_WITHOUT_ABORT(rtc_gpio_pullup_dis(ext_wakeup_pin_1));
        ESP_ERROR_CHECK_WITHOUT_ABORT(rtc_gpio_pulldown_en(ext_wakeup_pin_1));
    } else {
        ESP_ERROR_CHECK_WITHOUT_ABORT(rtc_gpio_pulldown_dis(ext_wakeup_pin_1));
        ESP_ERROR_CHECK_WITHOUT_ABORT(rtc_gpio_pullup_en(ext_wakeup_pin_1));
    }
    if (CONFIG_EXAMPLE_EXT1_WAKEUP_MODE_PIN_2) {
        ESP_ERROR_CHECK_WITHOUT_ABORT(rtc_gpio_pullup_dis(ext_wakeup_pin_2));
        ESP_ERROR_CHECK_WITHOUT_ABORT(rtc_gpio_pulldown_en(ext_wakeup_pin_2));
    } else {
        ESP_ERROR_CHECK_WITHOUT_ABORT(rtc_gpio_pulldown_dis(ext_wakeup_pin_2));
        ESP_ERROR_CHECK_WITHOUT_ABORT(rtc_gpio_pullup_en(ext_wakeup_pin_2));
    }
...
#else
    if (CONFIG_EXAMPLE_EXT1_WAKEUP_MODE) {
        ESP_ERROR_CHECK_WITHOUT_ABORT(gpio_pullup_dis(ext_wakeup_pin_1));
        ESP_ERROR_CHECK_WITHOUT_ABORT(gpio_pulldown_en(ext_wakeup_pin_1));
        ESP_ERROR_CHECK_WITHOUT_ABORT(gpio_pullup_dis(ext_wakeup_pin_2));
        ESP_ERROR_CHECK_WITHOUT_ABORT(gpio_pulldown_en(ext_wakeup_pin_2));
    } else {
        ESP_ERROR_CHECK_WITHOUT_ABORT(gpio_pulldown_dis(ext_wakeup_pin_1));
        ESP_ERROR_CHECK_WITHOUT_ABORT(gpio_pullup_en(ext_wakeup_pin_1));
        ESP_ERROR_CHECK_WITHOUT_ABORT(gpio_pulldown_dis(ext_wakeup_pin_2));
        ESP_ERROR_CHECK_WITHOUT_ABORT(gpio_pullup_en(ext_wakeup_pin_2));
    }
#endif
#endif
...
void deep_sleep_register_touch_wakeup(void)
{
    ...
    ESP_ERROR_CHECK_WITHOUT_ABORT(esp_sleep_enable_touchpad_wakeup());
    ESP_ERROR_CHECK_WITHOUT_ABORT(esp_sleep_pd_config(ESP_PD_DOMAIN_RTC_PERIPH, ESP_PD_OPTION_ON));
}
#endif // CONFIG_EXAMPLE_TOUCH_WAKEUP
//------------------------------------------------------------------------------------------------------------